...
description: |
  ...
  Codex memory, project quality gates, and skills ecosystem.
...
CLAUDE_MEMORY_ROOT="${CLAUDE_MEMORY_ROOT:-$HOME/.Codex/projects}"
CLAUDE_MEMORY_KEY="${CLAUDE_MEMORY_KEY:-}"
SKILLS_DIR="${SKILLS_DIR:-.Codex/skills}"
```
...
### 4a — Hook registered
```bash
grep -c "run_hook.sh" .Codex/settings.json 2>/dev/null \
...
[ -f AGENTS.md ] \
  && echo "AGENTS.md OK ($(wc -l < AGENTS.md) lines)" \
  || echo "AGENTS.md MISSING — add project-level AI instructions"
...
echo "=== Local skills (.Codex/skills/) ==="
...
Tier 2e — AGENTS.md & AGENTS.md [N/2]  results  ← cross-tool portability
...
Tier 7  — Codex Memory    [N/3]  results
...
If Tier 4a FAILED — add to `.Codex/settings.json`:
```json
{
  "hooks": {
    "PostToolUse": [{
...
PREREQUISITES
  ...
  - Codex     (required for /verify-ai-docs skill)
...
QUICK SETUP
  1. Scripts are already committed to the project repo.
     git pull   (gets tools/ai_docs/ + .Codex/skills/)

  2. Register the PostToolUse hook:
     Edit .Codex/settings.json and add the run_hook.sh command.
