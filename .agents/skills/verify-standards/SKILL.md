...
description: |
  ...
  across three enforcement tiers: CI gates, AI assistant rules (AGENTS.md),
...
## TIER 2 — AI Assistant Rules (AGENTS.md + AGENTS.md / project instructions)
...
```bash
# AGENTS.md at project root — cross-tool rules (Codex, Cursor, Codex, Copilot)
[ -f AGENTS.md ] && echo "AGENTS.md OK ($(wc -l < AGENTS.md) lines)" || echo "MISSING — run: cp /path/to/ai-native-dev-stack/AGENTS.md ."

# AGENTS.md references AGENTS.md via @include
[ -f AGENTS.md ] && grep -q "@AGENTS.md" AGENTS.md \
  && echo "@AGENTS.md present in AGENTS.md" \
  || echo "AGENTS.md does not reference @AGENTS.md — add '@AGENTS.md' near the top"
```

- [ ] `AGENTS.md` present at project root (cross-tool portability)
- [ ] `AGENTS.md` includes `@AGENTS.md` so rules load in Codex automatically

### 2b — Project-specific rules (AGENTS.md)

```bash
# AGENTS.md exists and is substantial
[ -f AGENTS.md ] && echo "EXISTS ($(wc -l < AGENTS.md) lines)" || echo "MISSING"

# Check for key sections (adapt to your project)
grep -c "Thread\|Real.time\|Forbidden\|Constraint" AGENTS.md 2>/dev/null && echo "RT rules present" || echo "RT rules missing"
grep -c "naming\|convention\|pattern" AGENTS.md 2>/dev/null && echo "Conventions present" || echo "Missing"
grep -c "pre.commit\|build\|test" AGENTS.md 2>/dev/null && echo "Build rules present" || echo "Missing"
grep -c "error\|unwrap\|Result\|catch" AGENTS.md 2>/dev/null && echo "Error handling present" || echo "Missing"
```
...
Tier 2 — AGENTS.md Rules     N/M  [results]
...
- If AGENTS.md is missing key sections → add them before next session
...
2. **Replace Tier 2 checklist** with your project's AGENTS.md sections
...
Save the customized skill as `.Codex/skills/verify-standards/SKILL.md` in your project.
